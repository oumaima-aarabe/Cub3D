...
/*   Updated: 2024/01/05 01:11:01 by ichaiq           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void detect_player(t_data * data)
{
	...
	while (data->map_data->mini_map[x])
	{
		...
		while (data->map_data->mini_map[x][y])
		{
			...
			{
				set_player_pos(x, y ,data->map_data->mini_map[x][y], data->map_data->player);
...
int	parse_args(char **av, t_data *data)
{
	...
	// ft_memset(data->ray_buffer, 0, sizeof(t_ray));
	// if (!data->ray_buffer)
	// 	return (0);
	// convert_dims_map(data->map_data);
	return (1);
}